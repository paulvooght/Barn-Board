...
- **2b-iii — stand Yonder up (photo + holds). ✅ DONE & deployed (2026-06-05).** Yonder is live: `boards` row `275dfaa7-1df9-4fe7-8332-c2795eb9ebe7` (slug `yonder`, **public**, owner/admin Paul; `claude-test` member), `holds_<id>` (103 auto-detected holds), `board_image_config_<id>`, `boards.specs.boardRegion {1,0.5,98,97}`, image in the bucket. Migration `006_yonder_board.sql` (+ executable twin `scripts/seed_yonder.mjs`, dry-run + `--commit`). All three carried-over follow-ups resolved:
  ...
  - First holds: `detect_holds.py` got an **expanded colour palette** (red/orange/green/blue/pink) + a **watershed split** for touching holds (67 → 103). ⚠️ Partial seed — **refine in Hold Manager before Yonder gets routes** (IDs freeze once referenced). New per-wall source-asset convention: `board-assets/<slug>/`.
- **2b-iv — onboarding (join public list / private join-code via a `SECURITY DEFINER` fn) + a "wall members / make admin" screen. ← THIS IS NEXT.** Note: Yonder is public but there's no in-app way to discover/join it yet — that's this phase. Also still pending from 2b-iii: a Hold-Manager **refine pass** on Yonder's 103 holds (do before it has routes).
