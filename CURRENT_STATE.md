...
Paul can't edit holds in **Hold Manager on laptop**. This is the current priority: it **blocks the Yonder refine pass**, which must precede Yonder getting routes (hold IDs freeze once a route references them). **Being fixed in a dedicated thread before multi-wall 2b-iv goes live** — 2b-iv opens Yonder to joiners/route-setters, so its go-live waits on this fix + the refine pass. **Symptom (Paul, laptop/mouse):** double-clicking a hold to enter **vertex-edit** leaves the view stuck in **pan** — the grab "hand" stays attached to the click point and only panning works; vertices can't be edited. Mouse-only (phones use the separate touch path), hence laptop-specific. **Strong hypothesis (verify first, don't blind-patch):** in `BoardSetupView.jsx` `handleMouseUp` (~L771) the `pendingHoldRef` "tap" branch (~L783-787) calls `handleHoldTap` and **`return`s without resetting `panDragRef.current.active = false`** — that reset only runs at the function end (~L795), which the early return skips. `handleMouseDown` arms `panDragRef.active = true` whenever you press on a hold (~L692). Net: after a click on a hold, pan stays armed, so the next `mousemove` pans (`handleMouseMove` ~L754); a double-click is two such taps → stuck in pan, vertex handles uninteractable. The synthesized-mouse branch (~L772) *does* clear it, which is why touch/phone is unaffected. **Likely fix:** clear `panDragRef.current.active` on every `handleMouseUp` exit (e.g. at the top) — but **verify it doesn't regress** hold-drag / lasso / the `handleClick`-on-mouseup path, and that single-click select + double-click vertex-edit both still work. ⚠️ Touch/coordinate/pan handling is "DO NOT CHANGE CASUALLY" (CLAUDE.md). **Verify on BOTH laptop (mouse) and phone (touch).**
