...
**Status (2026-06-05):** Phase 1 design **approved**. Decisions: one account → many walls (data siloed per wall); each wall public or private (private = join code); **any member sets routes**; per-wall **admin** role manages a wall (admin-driven wall setup, no self-serve yet); Barn holds **migrate into the DB** (keep `holds.json` as revert). Rollout: **2a (plumbing) ✅** · **2b-i (scoping + switcher + per-wall admin) ✅** · **2b-ii (holds & image per wall) ✅** · **2b-iii (stand Yonder up — photo ready) ← NEXT** · 2b-iv (onboarding/join + admin-assignment UI) · 2c (RLS tenant isolation + per-board roles server-side) · billing later.

➡️ **2b-iii context + carried-over follow-ups: read `MULTIWALL_2B_HANDOVER.md`** (per-board image naming, new-wall boardRegion setup, `publish_board_image.py` per-board).
