...
- **Wizard: `upload → crop → markCorners → confirm`, with a perspective warp to fix lens distortion.** Phone photos always have the board as a trapezoid; the warp straightens it so holds stay aligned.
- **`MarkCornersStep` (free quad)** — user places 4 pins (TL/TR/BL/BR) on the 4 **physical board corners** in the new photo. Pins move independently (NOT rectangle-locked) so any trapezoid shape is accepted. 44px hit targets, `lastTouchTimeRef` touch guard, pinch-zoom + single-finger pan, loupe magnifier during touch drags. Dashed outline connects the 4 pins so the user sees the marked quad.
- **Confirm step warps on entry** — computes `perspectiveWarp(croppedCanvas, srcQuad, dstQuad, oldImgW, oldImgH)` where `srcQuad` is the user's 4 pins and `dstQuad` is the 4 corners of `holdsData.boardRegion` projected into the old image's natural pixel space. Output canvas has the old image's dimensions so the physical board sits in exactly the same pixels as before. Shows the warped canvas with hold overlay. "Adjust corners" returns to pins preserved; "Looks right — Save" uploads the warped canvas.
- **`boardRegion` remains a single source of truth in `holds.json`** — not per-image. The warp makes sure the new image conforms to it.
- **Save path uploads the warped canvas** (not the cropped one) as the four responsive variants (full, 2000w, 1200w, 800w).
- **Cache busting** — `cacheVersion: Date.now()` written on every save; `?v=<cacheVersion>` appended to `imgSrc` and each `imgSrcSet` variant URL so browsers refetch after replace.
- **Warp runs inside `setTimeout(0)`** so the "Warping image…" spinner paints before the synchronous pixel loop begins. Expect 2–5s on phone for a 2000px-wide image.
...
## Recent Changes
- **2026-04-17 (Session 3 corrected)** — Mid-session course-correction after testing. The original Session-3 thesis (recalibrate per-image `boardRegion` instead of warping) didn't work: phone photos always have perspective distortion, so the board is a trapezoid and holds can't line up without a warp. Reverted per-image `boardRegion`, removed back-compat auto-prompt, restored `computeHomography` / `perspectiveWarp` helpers. Rewrote `MarkCornersStep` as a **free-quad** step (4 pins on physical board corners, no rectangle lock). Confirm step now runs a perspective warp so the user's quad maps to the fixed `boardRegion` rectangle, producing an output canvas at old-image dimensions. Save uploads the warped canvas. Cache-busting via `?v=<cacheVersion>` retained. Safe tag `v1.1-pre-session-3` preserved.
