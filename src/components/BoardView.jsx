                  ...
                  <mask id="hold-cutout-mask">
                    ...
                    {selectedHolds.map(hold => {
                      if (hold.polygon?.length >= 3) {
                        ...
                        return <polygon key={hold.id} points={pts} fill="black" strokeLinejoin="round" />;
                      }
                      const w = hold.w_pct !== undefined ? hold.w_pct : hold.r * 2;
                      const h = hold.h_pct !== undefined ? hold.h_pct : hold.r * 2;
                      return <ellipse key={hold.id} cx={toX(hold.cx)} cy={toY(hold.cy)} rx={Math.max((w / 100) * bW / 2, 2)} ry={Math.max((h / 100) * bH / 2, 2)} fill="black" />;
                    })}
                  </mask>
                </defs>
            ...
            >
              ...
              {dimBoard && holdSnapshots && (() => {
                ...
                return Object.entries(selection || {}).filter(([id]) => !holdIdSet.has(id)).map(([id]) => {
                  ...
                  if (snap.polygon?.length >= 3) {
                    ...
                    return (
                      <g key={`ghost-${id}`} style={{ pointerEvents: 'none' }}>
                        <polygon points={pts} fill="none" stroke="#FF1493" strokeWidth={Math.round(2 * pxScale)} strokeDasharray={`${Math.round(3.5 * pxScale)} ${Math.round(2 * pxScale)}`} strokeLinejoin="round" opacity={0.8} />
                  ...
                  return (
                    <g key={`ghost-${id}`} style={{ pointerEvents: 'none' }}>
                      <ellipse cx={gX(snap.cx)} cy={gY(snap.cy)} rx={rx} ry={ry} fill="none" stroke="#FF1493" strokeWidth={Math.round(2 * pxScale)} strokeDasharray={`${Math.round(3.5 * pxScale)} ${Math.round(2 * pxScale)}`} opacity={0.8} />
                      <text x={gX(snap.cx)} y={gY(snap.cy)} textAnchor="middle" dominantBaseline="central"
                        fontSize={Math.max(bWidth * 0.018, 14)} fontWeight="900" fill="#FF1493"
                        style={{ pointerEvents: 'none' }}
                      >✕</text>
