...
export default function App() {
  ...
  // Tap-to-segment candidate library for the ACTIVE wall (Hold Manager "Tap" tool).
  // Per-board (board-generic): generated by scripts/build_candidates.py and stored in
  // board_settings['hold_candidates_<boardId>']. Empty for walls that don't have one yet.
  const [tapCandidates, setTapCandidates] = useState([]);
  useEffect(() => {
    if (!activeBoardId) { setTapCandidates([]); return; }
    let cancelled = false;
    db.getBoardCandidates(activeBoardId)
      .then(({ data }) => { if (!cancelled) setTapCandidates(data?.data?.candidates || []); })
      .catch(() => { if (!cancelled) setTapCandidates([]); });
